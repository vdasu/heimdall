...
def main(c_file_path, c_ebpf_obj_path, entry_symbol, map_specs,
         ...
         pkt_size=None, last_k_compile_attempts=3, reasoning_effort=None):
...
if __name__ == "__main__":
    ...
    parser.add_argument("--provider",
                        choices=["anthropic", "anthropic-foundry", "openai", "ollama", "openai-compat"],
                        default="anthropic",
                        help="LLM provider (default: anthropic). Use anthropic-foundry "
                             "for Claude via Azure AI Foundry (--base-url + --api-key).")
    ...
    parser.add_argument("--reasoning-effort", default=os.environ.get("REASONING_EFFORT"),
                        help="Reasoning effort for reasoning models on OpenAI-compatible "
                             "endpoints (e.g. 'xhigh' for Azure gpt-5.4). When set, the "
                             "endpoint receives max_completion_tokens + reasoning_effort and "
                             "temperature is omitted. Also reads REASONING_EFFORT env var.")
