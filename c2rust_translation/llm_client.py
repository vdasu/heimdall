...
class LLMClient:
    ...
    def __init__(self, provider="anthropic", model=None, base_url=None, api_key=None,
        ...
        elif provider == "anthropic-foundry":
            import anthropic
            fkey = api_key or os.environ.get("ANTHROPIC_AZURE_API_KEY")
            furl = (base_url or os.environ.get("ANTHROPIC_AZURE_BASE_URL")
                    or "https://ebpf-us2.services.ai.azure.com/anthropic")
            if not fkey:
                raise ValueError("anthropic-foundry provider needs --api-key or "
                                 "ANTHROPIC_AZURE_API_KEY")
            self.client = anthropic.Anthropic(
                base_url=furl, api_key=fkey,
                timeout=3600.0, max_retries=3,
            )
    ...
    def chat(self, messages, max_tokens=16_384, temperature=0.7):
        ...
        t0 = time.monotonic()
        if self.provider in ("anthropic", "anthropic-foundry"):
