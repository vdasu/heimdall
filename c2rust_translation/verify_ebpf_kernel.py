"""
Verify eBPF bytecode against the Linux kernel verifier using direct bpf() syscall.

Unlike verify_ebpf_bytecode.py (which uses bpftool/libbpf), this script calls the
bpf() syscall directly via ctypes. This avoids libbpf's rejection of legacy 'maps'
sections (without dot prefix) that Aya-compiled binaries produce.

Usage:
    python verify_ebpf_kernel.py <program.o> [--prog <name>] [--type <type>] [--verbose]

Requires sudo (CAP_BPF) to load programs into the kernel.

Examples:
    # Auto-detect program type and verify all programs in the object
    sudo python verify_ebpf_kernel.py my_prog.o

    # Verify a specific program by name
    sudo python verify_ebpf_kernel.py my_prog.o --prog bpf_prog1

    # Override the program type
    sudo python verify_ebpf_kernel.py my_prog.o --type tracepoint

    # Show full verifier log even on success
    sudo python verify_ebpf_kernel.py my_prog.o --verbose

    # Dry-run: just list detected programs and maps without loading
    python verify_ebpf_kernel.py my_prog.o --dry-run
"""
...
def _kconfig_symbol_values():
    """
    Supported __kconfig extern symbols and their raw bytes.
    """
    return {
...
def bpf_syscall(cmd, attr_buf, attr_size):
    """
    Call bpf(cmd, attr, size) syscall. Returns the syscall return value.
...
def _parse_cpu_list(cpu_list_text):
    """Parse Linux cpu list format (e.g. '0-3,8,10-11') into count."""
    total = 0
...
def _n_possible_cpus():
    """Best-effort possible CPU count for map defaults."""
...
def _default_max_entries_for_map_type(map_type):
    """
    Fallback max_entries for objects that intentionally leave it as 0 and rely
    on userspace to patch it before load.
    """
    if map_type in {
        BPF_MAP_TYPE_HASH,
...
def find_programs(filepath):
    """
    Parse ELF to find eBPF program sections and their kernel prog_type.
    Returns list of tuples:
      (display_name, section_name, prog_type_int, entry_off, entry_size)
    """
    programs = []
...
def _find_section_entry_functions(elf, symtab, section_name):
    """
    Return sorted list of entry-like function symbols in `section_name`:
      [(func_name, st_value, st_size), ...]

    We keep GLOBAL/WEAK symbols only, because local STT_FUNC entries typically
    represent internal subprograms rather than top-level section entries.
    """
    entries = []
...
def _section_to_prog_type(name_lower):
    """Map a section name to BPF_PROG_TYPE_* integer, or None."""
...
def _prog_type_name(prog_type):
    """Human-readable name for a BPF_PROG_TYPE_* constant."""
...
def _parse_raw_btf_name_ids(filepath):
    """
    Parse a raw-BTF blob (e.g. /sys/kernel/btf/vmlinux) and return:
      {"func": {func_name: btf_type_id},
       "typedef": {typedef_name: btf_type_id}}
    """
    with open(filepath, "rb") as f:
        data = f.read()
...
def _load_vmlinux_btf_name_ids():
    """Load and cache vmlinux BTF function/typedef IDs from /sys/kernel/btf/vmlinux."""
...
def _resolve_tracing_attach(section_name):
    """
    Resolve expected_attach_type + attach_btf_id for tracing-style sections.

    Returns (expected_attach_type, attach_btf_id, resolved_func_name) or
    (0, 0, None) if no attach resolution is required.
    Raises RuntimeError on unresolved required attach metadata.
    """
...
def _is_dummy_tracing_section(section_name):
    """
    libbpf-tools objects often include placeholder fentry/fexit programs with
    synthetic attach names (e.g. 'dummy_file_read') that userspace loaders
    rewrite at runtime. They are not directly loadable as-is.
    """
    if "/" not in section_name:
        return False
    prefix, attach_name = section_name.split("/", 1)
...
def find_maps_btf(filepath):
    """
    Try to extract map definitions from BTF. Accepts both '.maps' and 'maps'
    DATASEC names.
    Returns list of MapMetadata or empty list on failure.
    """
    try:
...
def find_maps_legacy(filepath):
    """
    Parse legacy 'maps' section as array of struct bpf_map_def.
    Correlate with symbol table to get map names.

    The struct size varies: older kernels/programs use 20 bytes (5 × u32:
    type, key_size, value_size, max_entries, map_flags) while newer ones use
    28 bytes (7 × u32, adding inner_map_idx and numa_node). Auto-detect the
    actual stride from symbol table offsets; fall back to LEGACY_MAP_DEF_SIZE.

    Returns list of MapMetadata.
    """
    maps = []
    with open(filepath, "rb") as f:
        elf = ELFFile(f)
...
def find_maps(filepath):
    """
    Discover maps in the ELF file. Merges BTF and legacy results.

    Some programs (e.g. bmc-cache) use both modern BTF-style .maps and legacy
    struct bpf_map_def SEC("maps") in the same binary. Always parse both and
    merge by name (BTF takes precedence on collision). Fall back to legacy-only
    if BTF returns no valid maps.
    """
...
def _get_kernel_version():
    """Get the running kernel version encoded as KERNEL_VERSION(a,b,c)."""
...
def _get_symbol_section_name(elf, symbol):
    """Get the section name that a symbol belongs to."""
...
def _is_global_data_section(section_name):
    """
    Return True when an ELF section should be treated as map-backed global data.
    Accepts canonical sections (e.g. ".rodata") and subsection variants
    (e.g. ".rodata.cst16", ".data.rel.ro", ".bss.foo").
    """
    if not section_name:
        return False
...
def collect_relocations(filepath, section_name):
    """
    Collect all relocations for a program section, classified by type.

    Returns a dict with keys:
        "map":     [(offset, sym_name), ...]   — R_BPF_64_64 refs to maps section
        "kconfig": [(offset, sym_name), ...]   — R_BPF_64_64 refs to .kconfig
        "data":    [(offset, sym_name, sym_section, sym_value), ...]  — .rodata/.data/.bss (+ subsections)
        "call":    [(offset, sym_name, sym_value), ...]  — R_BPF_64_32 refs to .text
        "kfunc":   [(offset, sym_name), ...]   — R_BPF_64_32 refs to external kernel functions
    """
...
def get_text_functions(filepath):
    """
    Get function symbols defined in .text section.
    Returns dict: func_name → (byte_offset_in_text, byte_size).
    """
    funcs = {}
    with open(filepath, "rb") as f:
        elf = ELFFile(f)
...
def _build_text_func_offset_index(text_funcs):
    """
    Build mapping: byte_offset_in_text -> function name.
    If multiple names share the same offset, keep the first one.
    """
    offset_to_name = {}
...
def _resolve_text_call_target(sym_name, sym_value, text_funcs, text_offset_to_name):
    """
    Resolve a call relocation target into a concrete .text function.

    Returns:
        (func_name, func_start_off) or (None, None) if unresolved.
    """
...
def _build_text_func_ranges(text_funcs):
    """Return sorted [(start_off, end_off, func_name), ...] for .text functions."""
    ranges = []
...
def _find_text_func_for_offset(byte_off, text_func_ranges):
    """Resolve a byte offset inside .text to the containing function name."""
    off = int(byte_off)
...
def _shift_relocations(relocs, byte_offset_delta):
    """
    Return a copy of relocs where each relocation offset is shifted by delta.
    Useful when appending one ELF section's bytecode after another.
    """
...
def _extend_relocations(dst, src):
    """Append relocation lists from src into dst in-place."""
    for k in ("map", "kconfig", "data", "call", "kfunc"):
        dst.setdefault(k, []).extend(src.get(k, []))

def _slice_relocations(relocs, start_off, size):
    """
    Keep only relocations that land inside [start_off, start_off + size),
    and normalize their offsets to the sliced program base.
    """
    if size is None:
        return relocs
...
def read_section_data(filepath, section_name):
    """Read raw bytes of a section. Returns None if section doesn't exist."""
    with open(filepath, "rb") as f:
        elf = ELFFile(f)
...
def read_section_blob(filepath, section_name):
    """
    Read full section contents as bytes suitable for backing global-data maps.
    For SHT_NOBITS sections (e.g. .bss), returns zero-initialized bytes of sh_size.
...
def _inner_map_type_name_to_int(type_name):
    """Reverse-map a map type name string to its BPF_MAP_TYPE_* integer."""
...
def _fixup_btf_datasec(btf_data, filepath):
    """
    Patch DATASEC types in raw BTF data to match what libbpf does before
    BPF_BTF_LOAD:

    1. Set DATASEC *size* from ELF section sizes (clang emits 0).
    2. Set DATASEC VAR *offsets* from ELF symbol table (clang emits 0 for
       .maps vars).

    Without these fixups the kernel rejects the BTF with EINVAL.
    """
...
def load_btf_from_elf(filepath):
    """
    Read the .BTF section from an ELF file, fix up DATASEC sizes, and load
    it into the kernel via bpf(BPF_BTF_LOAD).  Returns the BTF fd, or -1.
    """
    with open(filepath, 'rb') as f:
        elf = ELFFile(f)
...
def find_rodata_btf_type_ids(filepath):
    """
    Parse BTF from the ELF to find:
      - key_type_id:   a BTF_KIND_INT with size=4  (u32 / int / __u32)
      - value_type_id: the BTF_KIND_DATASEC named '.rodata'

    Returns (key_type_id, value_type_id), or (0, 0) if not found.
    """
    try:
        parser = BTFParser(filepath)
...
def find_map_btf_type_ids(filepath):
    """
    Parse BTF from the ELF to find key/value BTF type IDs for each named map
    in the .maps DATASEC.

    Required for maps whose value struct contains bpf_spin_lock: the kernel
    rejects BPF_MAP_CREATE for such maps unless btf_fd + btf_key_type_id +
    btf_value_type_id are provided so it can locate the spin_lock offset.

    Returns {map_name: (key_type_id, value_type_id)}.
    """
    try:
        parser = BTFParser(filepath)
    except (ValueError, FileNotFoundError):
...
def create_map(map_meta, map_flags=0, btf_fd=-1, btf_key_type_id=0,
               btf_value_type_id=0):
    """
    Create a BPF map via bpf(BPF_MAP_CREATE).
    Returns the map file descriptor, or raises RuntimeError.

    For array_of_maps / hash_of_maps, creates a temporary inner map template
    and passes its fd via the inner_map_fd field (offset 20) in bpf_attr.
    """
    attr_size = 128
    attr = (ctypes.c_char * attr_size)()
    ctypes.memset(attr, 0, attr_size)
...
def _patch_lddw_map_value(bytecode, offset, map_fd, value_offset):
    """
    Patch an LDDW instruction for BPF_PSEUDO_MAP_VALUE.
    The verifier resolves this to a direct pointer into the map value memory.
    First imm = map_fd, second imm = offset within the value.
    """
...
def map_update_elem(map_fd, key_bytes, value_bytes):
    """
    Update a map element via bpf(BPF_MAP_UPDATE_ELEM).
    """
...
def map_lookup_elem(map_fd, key_bytes, value_size):
    """Look up a map element via bpf(BPF_MAP_LOOKUP_ELEM).

    Returns the value bytes (length value_size) on hit, or None if the key is
    absent (ENOENT). Used by the Tier-1b differential test to read back the
    final map state after BPF_PROG_TEST_RUN.
    """
    key_buf = (ctypes.c_char * len(key_bytes)).from_buffer_copy(key_bytes)
    val_buf = (ctypes.c_char * value_size)()
    ctypes.memset(val_buf, 0, value_size)
    ...
    ret = bpf_syscall(BPF_MAP_LOOKUP_ELEM, attr, attr_size)
    if ret < 0:
        errno = ctypes.get_errno()
        if errno == 2:
            return None
        raise RuntimeError(
            f"BPF_MAP_LOOKUP_ELEM failed: errno={errno} ({os.strerror(errno)})"
        )
    return bytes(val_buf[:value_size])

def create_kconfig_map(kconfig_relocs):
    """
    Create an internal BPF_MAP_TYPE_ARRAY map to hold .kconfig values.
    Populates it with resolved kconfig values.

    Returns (map_fd, sym_name_to_offset) or (None, {}) if no kconfig relocs.
    """
...
def create_data_section_maps(filepath, data_relocs, overrides=None):
    """
    Create internal maps backing global data sections referenced by relocations.

    overrides: optional {section_name: {symbol_name: bytes}} that splices the
      given byte values into the section blob at the symbol's offset BEFORE
      map_update_elem + BPF_MAP_FREEZE. Used to set `const volatile` globals
      (e.g. opensnoop's targ_pid / targ_tgid) at load time. Override bytes
      must already be the correct length for the symbol's type.

    Returns:
      (sec_to_fd, sec_to_size)
      where sec_to_fd maps section name ('.rodata', '.data', '.bss', and subsections) to map fd,
      and sec_to_size stores each section's byte size.
    """
    sec_to_fd = {}
    sec_to_size = {}
...
def _patch_lddw_imm(bytecode, offset, imm_value, src_reg=0):
    """
    Patch an LDDW instruction (16 bytes) at the given offset.
    Sets imm in the first slot and zeroes the second slot's imm.
    Optionally sets src_reg (e.g. 1 for BPF_PSEUDO_MAP_FD).
    """
    dst_reg = bytecode[offset + 1] & 0x0f
...
def apply_relocations(
    ...
    """
    Apply all relocations to the bytecode.

    Args:
        bytecode: mutable bytearray of the combined program+subprogram bytecode
        relocs: dict from collect_relocations()
        map_name_to_fd: map symbol name → file descriptor
        func_insn_offsets: dict mapping .text func_name → insn index in combined bytecode
        func_insn_offsets_by_off: dict mapping .text byte offset → insn index
        kconfig_fd: FD for the internal kconfig map (or None)
        kconfig_offsets: dict mapping kconfig sym_name → byte offset in map value
        data_sec_to_fd: dict mapping data section name to backing map fd
        data_sec_to_size: dict mapping data section name to backing map size
    """
...
def load_program(
    ...
    Returns (fd, verifier_log) on success, or (-1, verifier_log) on failure.
    """
...
def assemble_program_bytecode(
    ...
    """
    Read program bytecode from ELF, append .text subprograms, apply relocations.

    Returns (bytecode, error_msg).  bytecode is None on failure.
    """
...
def verify_program(
    ...
    """
    Verify a single eBPF program section.
    Returns (success, log_str, skipped).
    """
...
def load_program_to_fd(obj_file, prog_filter=None, verbose=False, return_maps=False):
    """Load one program section from obj_file into the kernel and return it.

    Returns (prog_fd, fds_to_close, prog_type, display_name). Raises
    RuntimeError on any failure. Caller must close prog_fd and every fd in
    fds_to_close. Mirrors main()'s map setup + assemble + load sequence.

    If return_maps=True, returns a 5-tuple with a trailing dict
    {map_name: map_fd} so callers (e.g. the Tier-1b differential test) can seed
    and read back map state. The map fds are also in fds_to_close.
    """
    programs = find_programs(obj_file)
    if not programs:
        raise RuntimeError(f"no loadable program sections in {obj_file}")
    if prog_filter:
        programs = [p for p in programs if prog_filter in p[0] or prog_filter in p[1]]
        if not programs:
            raise RuntimeError(f"no program matching {prog_filter!r} in {obj_file}")
    display_name, section_name, prog_type, entry_off, entry_size = programs[0]

    all_kconfig_relocs, all_data_relocs = [], []
    relocs = _slice_relocations(collect_relocations(obj_file, section_name),
                               entry_off, entry_size)
    all_kconfig_relocs.extend(relocs["kconfig"])
    all_data_relocs.extend(relocs["data"])
    text_relocs = collect_relocations(obj_file, ".text")
    all_kconfig_relocs.extend(text_relocs["kconfig"])
    all_data_relocs.extend(text_relocs["data"])

    fds_to_close = []
    map_name_to_fd = {}
    kconfig_fd, kconfig_offsets = None, {}
    data_sec_to_fd, data_sec_to_size = {}, {}

    map_btf_fd = load_btf_from_elf(obj_file)
    if map_btf_fd >= 0:
        fds_to_close.append(map_btf_fd)
    map_btf_type_ids = find_map_btf_type_ids(obj_file) if map_btf_fd >= 0 else {}

    for m in find_maps(obj_file):
        if m.map_type == 0 and m.key_size == 0 and m.value_size == 0:
            continue
        btf_key_tid, btf_val_tid = map_btf_type_ids.get(m.name, (0, 0))
        if m.map_type in (BPF_MAP_TYPE_PERF_EVENT_ARRAY, BPF_MAP_TYPE_PROG_ARRAY,
                          BPF_MAP_TYPE_CGROUP_ARRAY, BPF_MAP_TYPE_STACK_TRACE):
            btf_key_tid, btf_val_tid = 0, 0
        fd = create_map(m, btf_fd=map_btf_fd if btf_key_tid else -1,
                        btf_key_type_id=btf_key_tid, btf_value_type_id=btf_val_tid)
        map_name_to_fd[m.name] = fd
    ...
    if all_kconfig_relocs:
        kconfig_fd, kconfig_offsets = create_kconfig_map(all_kconfig_relocs)
        if kconfig_fd is not None:
            fds_to_close.append(kconfig_fd)
    if all_data_relocs:
        data_sec_to_fd, data_sec_to_size = create_data_section_maps(obj_file, all_data_relocs)
        fds_to_close.extend(data_sec_to_fd.values())

    bytecode, err = assemble_program_bytecode(
        obj_file, section_name, entry_off, entry_size, map_name_to_fd,
        kconfig_fd, kconfig_offsets, data_sec_to_fd, data_sec_to_size)
    if bytecode is None:
        for fd in fds_to_close:
            try: os.close(fd)
            except OSError: pass
        raise RuntimeError(f"bytecode assembly failed: {err}")

    fd, vlog, errno = load_program(prog_type, bytecode,
                                   log_level=4 if verbose else 1)
    if fd < 0:
        for f in fds_to_close:
            try: os.close(f)
            except OSError: pass
        raise RuntimeError(f"BPF_PROG_LOAD failed (errno={errno} "
                           f"{os.strerror(errno)}):\n{vlog}")
    if return_maps:
        return fd, fds_to_close, prog_type, display_name, map_name_to_fd
    return fd, fds_to_close, prog_type, display_name

def prog_test_run(prog_fd, data_in, data_out_size=4096, repeat=1):
    """Run BPF_PROG_TEST_RUN on prog_fd with data_in. Returns (retval, data_out).

    Raises RuntimeError on syscall failure.
    """
    data_in_buf = (ctypes.c_char * max(1, len(data_in)))(*data_in)
    data_out_buf = (ctypes.c_char * data_out_size)()

    attr = (ctypes.c_char * 96)()
    struct.pack_into("<I", attr, 0, prog_fd)
    struct.pack_into("<I", attr, 8, len(data_in))
    struct.pack_into("<I", attr, 12, data_out_size)
    struct.pack_into("<Q", attr, 16, ctypes.addressof(data_in_buf))
    struct.pack_into("<Q", attr, 24, ctypes.addressof(data_out_buf))
    struct.pack_into("<I", attr, 32, repeat)

    ret = bpf_syscall(BPF_PROG_TEST_RUN, attr, len(attr))
    if ret < 0:
        errno = ctypes.get_errno()
        raise RuntimeError(f"BPF_PROG_TEST_RUN failed: errno={errno} "
                           f"({os.strerror(errno)})")
    retval = struct.unpack_from("<I", attr, 4)[0]
    data_size_out = struct.unpack_from("<I", attr, 12)[0]
    return retval, bytes(data_out_buf[:data_size_out])
