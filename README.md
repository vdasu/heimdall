...
│   ├── run_translation_commands.txt   115 ready-to-run invocations
...
│   ├── c_bpf_programs/                115 C eBPF programs (.c + .o)
│   └── verified_translations/         109 verified Aya Rust translations (.rs + .o)
├── environment.yml         conda env spec (Python 3.10 + angr stack + Z3 + LLM SDKs)
├── README.md               (this file)
└── LICENSE
```

The dataset holds 115 valid eBPF programs. Heimdall (Agentic, opus-4.6)
produces 109 fully verified translations. Of the remaining six, three are
partially verified (some entry points verified, the rest exceeding the
symbolic-execution time/memory limits) and three are unverified (all entry
points exceed solver limits); none are true translation failures. The 109
fully verified translations are included in `verified_translations/`.
...
## Quick start: re-verify one of the 109 translations
...
python verify_mixed_entries.py \
    c_bpf_programs/KEN/dataset/libbpf/kfree_skb.bpf.o \
    verified_translations/ken_libbpf__kfree_skb/ken_libbpf__kfree_skb.o \
    kfree_skb kfree_skb \
    kfree_skb_total:hash
```

A passing run prints `equivalent: True` for the entry symbol.

### Expected divergences

Not every translation is *supposed* to verify equivalent and the buggy programs (8 programs in the verified folder) that are fixed by Heimdall will diverge. Refer to the paper for more details.
...
ready-to-run command for each of the 115 C programs, using
...
## Reproducing the paper: 115-program agentic sweep

`agent_code_translate.sh` is the driver that runs an LLM agent over the
full 115-program set, capturing per-program metadata (wall time, token
